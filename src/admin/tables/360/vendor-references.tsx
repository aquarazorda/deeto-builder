...
const columns: ColumnDef<AccountContactWithAuthenticatedUser>[] = [
  ...
  },
  {
    ...
    accessorFn: ({ title, account }) =>
      `${capitalize(title)} / ${capitalize(account?.companyName)}`,
  },
  {
    header: "Email",
  ...
  },
  {
    header: "Status",
    accessorFn: ({ authenticatedUser }) =>
      capitalize(authenticatedUser.userStatus),
  },
  {
    id: "actions",
    cell: ({ row }) => {
...
export default function VendorReferencesTable() {
  ...
  return (
    <DataTable data={data.references} columns={columns} className="px-2" />
  );
}
